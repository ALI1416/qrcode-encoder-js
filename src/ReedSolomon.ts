...
function Encoder(coefficients: number[], degree: number): number[] {
  let info: GenericGFPoly = new GenericGFPoly(coefficients)
  info = info.MultiplyByMonomial(degree, 1)
  let remainder: GenericGFPoly = info.RemainderOfDivide(GenericGFPolyArray[degree])
  // 纠错码
  let result: number[] = remainder.Coefficients
  let length: number = result.length
  // 长度不够前面补0
  let padding: number = degree - length
  if (padding === 0) {
    return result
  } else {
